...
resource "azurerm_network_security_group" "nsg" {
  ...
  #    destination_address_prefix = azurerm_subnet.vnet_subnet.address_prefix
  #  }
}
...
resource "azurerm_monitor_autoscale_setting" "autoscaling" {
  ...
  profile {
    ...
    #rule {
    #  metric_trigger {
    #    metric_name        = var.req_autoscale_profile_info[count.index].profile_rules_metric_name
    #    metric_resource_id = azurerm_linux_virtual_machine_scale_set.vmss.id
    #    time_grain         = var.req_autoscale_profile_info[count.index].profile_rules_time_grain
    #    statistic          = var.req_autoscale_profile_info[count.index].profile_rules_statistic
    #    time_window        = var.req_autoscale_profile_info[count.index].profile_rules_time_window
    #    time_aggregation   = var.req_autoscale_profile_info[count.index].profile_rules_time_aggregation
    #    operator           = var.req_autoscale_profile_info[count.index].profile_rules_operator
    #    threshold          = var.req_autoscale_profile_info[count.index].profile_rules_threshold
    #  }
    #
    #  scale_action {
    #    direction = var.req_autoscale_profile_info[count.index].profile_rules_direction
    #    type      = var.req_autoscale_profile_info[count.index].profile_rules_type
    #    value     = var.req_autoscale_profile_info[count.index].profile_rules_value
    #    cooldown  = var.req_autoscale_profile_info[count.index].profile_rules_cooldown
    #  }
    #}
    #
    #rule {
...
#    command = "Start-Process iexplore ${azurerm_public_ip.public_ip.fqdn}"
#    
...
#    azurerm_monitor_autoscale_setting.autoscaling,
